...
public class ResgateDAO {
	...
	public void insere(Connection con, String usuario, Resgate obj) {

		try {

			int max = getMax(con, usuario, obj.getFichaConta(), obj.getData());

			stmtAux = con.prepareStatement("insert into " + usuario + ".CBPRESGATE "
			...
			System.err.println("Erro de Inclusão Resgate Bancários");
			e.printStackTrace();
		}
	}
	...
	private int getMax(Connection con, String usuario, int ficha, Date data) {
		...
		try {
			stmtAux = con.prepareStatement(
					"Select MAX( NUMERO ) From " + usuario + ".CBPRESGATE Where FICHACONTA = ? And DATA = ? ");
	...
	public void delete(Connection con, String usuario) {
		...
		try {
			stmtAux = con.prepareStatement("Delete from " + usuario + ".CBPRESGATE ");
			stmtAux.executeUpdate();
			stmtAux.close();
		} catch (SQLException e) {
			System.err.println("Erro de Exclusão Resgate Bancários");
			e.printStackTrace();
		}
	}
	...
	public void delete(Connection con, String usuario, Date ano) {
		...
		try {
			stmtAux = con.prepareStatement("Delete from " + usuario + ".CBPRESGATE where ANOFATO = ? ");
